...
W2 = randn(3, 128)
vt = reshape(v,1,3)
model3 = Chain(Dense(W1, true, relu), Dense(W2, true, relu), Dense(vt, false, identity))
loss4(x,y) = (1/2)*(model3(x)[1]-y)^2
f_layer = loss4(X[i,:],y[i])
g_layer = gradient(params(model3)) do
	loss4(X[i,:],y[i])
end
